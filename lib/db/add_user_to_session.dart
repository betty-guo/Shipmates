...
void addUserToSession(String userId, ActiveSessions session) async {
  var url = Uri.http('localhost:5000', 'active-sessions', {'id': session.id});
  Map<String, String> headersInfo = {"Content-type": "application/json"};
  Object bodyContent = jsonEncode({
    'num_people': 2,
    'users': [session.host, userId],
  });
  var response = await http.patch(url, headers: headersInfo, body: bodyContent);
